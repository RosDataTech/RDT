...
url = "https://new.mipt.ru/sveden/education/eduop"
...
for spec in eduOp:
    code = spec.find_all(itemprop = 'eduCode')[0]
    if code.text.strip() in it_codes:
        links = spec.find_all('a')
        link_in_spec_counter = 0
        for link in links:
            if f'OOP_{relevant_years[0]}'in link['href'] and '.pdf' in link['href']:
                print(code.text.strip(), 'https://new.mipt.ru' + link['href'])
                cur.execute(f'''INSERT INTO IT (university,code,url,type)
                                    VALUES ('MFTI','{code.text.strip()}','{'https://new.mipt.ru' + link['href']}','.pdf')''')
                link_in_spec_counter+=1
                pdf_counter+=1
        if link_in_spec_counter == 0:
            for link in links:
                if f'OOP_{relevant_years[1]}'in link['href'] and '.pdf' in link['href']:
                    print(code.text.strip(), 'https://new.mipt.ru' + link['href'])
                    cur.execute(f'''INSERT INTO IT (university,code,url,type)
                                        VALUES ('MFTI','{code.text.strip()}','{'https://new.mipt.ru' + link['href']}','.pdf')''')
                    pdf_counter+=1
